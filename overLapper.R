...
###########################################
vennPlot <- function(counts=counts, mymain="Venn Diagram", mysub="default",data.name=c("S1","S2","S3","S4","S5"), setlabels="default", yoffset=seq(0,10,by=0.34), ccol=rep(1,31), colmode=1, lcol=c("#FF0000", "#008B00", "#0000FF", "#FF00FF", "#CD8500"), lines=c("#FF0000", "#008B00", "#0000FF", "#FF00FF", "#CD8500"), mylwd=3, diacol=1, type="ellipse", ccex=1.0, lcex=1.0, ...) {
	...
	if(length(counts[[1]])==3) {
		...
		if(mysub=="default") {
			...
			mysub <- paste(paste("Unique objects: All =", sum(counts[[1]])), paste("; ",data.name[1]," =", sample_counts[1]), paste("; ",data.name[2]," =", sample_counts[2]), sep="")
		} else { 
			mysub <- mysub 
		}
	...
	if(length(counts[[1]])==7) { 
		...
		if(mysub=="default") {
			...
			mysub <- paste(paste("Unique objects: All =", sum(counts[[1]])), paste("; ",data.name[1]," =", sample_counts[1]), paste("; ",data.name[2]," =", sample_counts[2]), paste("; ",data.name[3]," =", sample_counts[3]), sep="")
		} else { 
			mysub <- mysub
		}
	...
	if(length(counts[[1]])==15 & type=="ellipse") {
		...
		if(mysub=="default") {
			...
			mysub <- paste(paste("Unique objects: All =", sum(counts[[1]])), paste("; ",data.name[1]," =", sample_counts[1]), paste("; ",data.name[2]," =", sample_counts[2]), paste("; ",data.name[3]," =", sample_counts[3]), paste("; ",data.name[4]," =", sample_counts[4]), sep="")
		} else { 
			mysub <- mysub
		}
	...
	if(length(counts[[1]])==31) {
		...
		if(mysub=="default") {
			...
			mysub <- paste(paste("Unique objects: All =", sum(counts[[1]])), paste("; ",data.name[1]," =", sample_counts[1]), paste("; ",data.name[2]," =", sample_counts[2]), paste("; ",data.name[3]," =", sample_counts[3]), paste("; ",data.name[4]," =", sample_counts[4]), paste("; ",data.name[5]," =", sample_counts[5]), sep="")
		} else { 
			mysub <- mysub
		}
